...
class _PollsScreenState extends ConsumerState<PollsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final canCreate = ref.watch(effectiveHasManagePermissionProvider) ||
        ref.watch(effectiveIsPartLeaderProvider);

    return Scaffold(
      appBar: AppBar(
        title: Text('참석 투표', style: AppText.headline(20)),
        actions: [
          if (canCreate)
            IconButton(
      ...
      body: pollsAsync.when(
        ...
        data: (polls) {
          ...
          final filtered = polls.where((p) {
            if (isAdmin) return true;
            final scope = p['scopePart'];
            return scope == null || scope == userPart;
          }).where((p) => _showOpen ? p['isOpen'] == true : p['isOpen'] != true).toList();

          return Column(children: [
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
              child: Row(children: [
                _tabChip('진행 중', _showOpen, () => setState(() => _showOpen = true)),
                const SizedBox(width: 8),
                _tabChip('마감됨', !_showOpen, () => setState(() => _showOpen = false)),
              ]),
            ),
            Expanded(
              child: filtered.isEmpty
                  ? Center(child: Text(_showOpen ? '진행 중인 투표가 없습니다' : '마감된 투표가 없습니다',
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (_, i) => _PollCard(
                        ...
                        onTap: () => setState(() {
                          _selectedPollId = _selectedPollId == filtered[i]['id'] ? null : filtered[i]['id'];
                        }),
                        onVote: (choice) => _handleVote(filtered[i]['id'], choice),
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(choice == 'attend' ? '참석으로 투표했습니다' : '불참으로 투표했습니다')),
        );
      }
    } catch (e) {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('마감', style: TextStyle(color: Colors.red))),
        ],
      ),
    );
    if (confirmed == true) {
        ...
        return AlertDialog(
          ...
          content: Column(mainAxisSize: MainAxisSize.min, children: [
            TextField(
              ...
              decoration: const InputDecoration(labelText: '제목', hintText: '예: 4/21 주일 참석 여부'),
            ),
...
class _PollCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final scopeLabel = scopePart != null ? '${User.partLabels[scopePart] ?? scopePart} 파트' : '전체';
    ...
    final canClose = isOpen && (profile?.isAdmin == true ||
        (profile?.isPartLeader == true && poll['createdBy'] == profile?.id));
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              Container(
                ...
                child: Text(scopeLabel, style: AppText.body(11, weight: FontWeight.w700, color: AppColors.secondary)),
              ),
            ]),
            const SizedBox(height: 10),
            Text(poll['title'] ?? '', style: AppText.headline(17)),
            const SizedBox(height: 4),
            Text(poll['targetDate'] ?? '', style: AppText.body(13, color: AppColors.muted)),
            const SizedBox(height: 12),
            Row(children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  border: Border.all(color: myChoice == 'attend' ? AppColors.success :
                      myChoice == 'absent' ? AppColors.error : AppColors.muted),
                  borderRadius: BorderRadius.circular(8),
                ),
                child: Text(
                  '내 투표: ${myChoice == 'attend' ? '참석' : myChoice == 'absent' ? '불참' : '미투표'}',
                  style: AppText.body(11, weight: FontWeight.w700,
                      color: myChoice == 'attend' ? AppColors.success :
                          myChoice == 'absent' ? AppColors.error : AppColors.muted),
                ),
              ),
            ]),
            if (isSelected) ...[
              ...
              if (isOpen)
                Padding(
                  ...
                  child: Text('마감 전까지 변경 가능', textAlign: TextAlign.center,
                      style: AppText.body(11, color: AppColors.muted)),
                ),
              if ((profile?.isAdmin ?? false) || (profile?.isPartLeader ?? false))
                _voterList(votes),
              if (canClose)
                Padding(
  ...
  Widget _voterList(List<Map<String, dynamic>> votes) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const SizedBox(height: 12),
      Text('투표자 상세', style: AppText.body(12, weight: FontWeight.w700, color: AppColors.muted)),
      const SizedBox(height: 6),
      ...filtered.map((v) => Padding(
        padding: const EdgeInsets.symmetric(vertical: 4),
        child: Row(children: [
          Text(v['userName'] ?? '', style: AppText.body(14, weight: FontWeight.w600)),
          const SizedBox(width: 8),
          Text(User.partLabels[v['userPart']] ?? '', style: AppText.body(11, color: AppColors.muted)),
          const Spacer(),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
            decoration: BoxDecoration(
              color: (v['choice'] == 'attend' ? AppColors.success : AppColors.error).withValues(alpha: 0.1),
              borderRadius: BorderRadius.circular(6),
            ),
            child: Text(
              v['choice'] == 'attend' ? '참석' : '불참',
              style: AppText.body(11, weight: FontWeight.w700,
                  color: v['choice'] == 'attend' ? AppColors.success : AppColors.error),
            ),
          ),
        ]),
      )),
