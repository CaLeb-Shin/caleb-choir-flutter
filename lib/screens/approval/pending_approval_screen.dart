                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _InfoRow(
                          label: '신청 유형',
                          value: profile.requestedRoleLabel,
                        ),
                        if (profile.generation != null &&
                            profile.generation!.isNotEmpty) ...[
                          const SizedBox(height: 10),
                          _InfoRow(label: '기수', value: profile.generation!),
                        ],
                        if (profile.choirName != null &&
                            profile.choirName!.isNotEmpty) ...[
                          const SizedBox(height: 10),
                          _InfoRow(label: '찬양대', value: profile.choirName!),
                        ],
                        if (profile.churchPosition != null &&
                            profile.churchPosition!.isNotEmpty) ...[
                          const SizedBox(height: 10),
                          _InfoRow(
                            label: '교회 직분',
                            value: profile.churchPosition!,
                          ),
                        ],
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        SizedBox(
          width: 80,
          child: Text(label, style: AppText.body(13, color: AppColors.muted)),
        ),
        Expanded(
          child: Text(value, style: AppText.body(14, weight: FontWeight.w700)),
        ),
      ],
    );
  }
