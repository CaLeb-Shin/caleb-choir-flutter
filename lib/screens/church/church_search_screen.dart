...
class _ChurchSearchScreenState extends ConsumerState<ChurchSearchScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final title = widget.requestedRole == 'part_leader' ? '파트장 신청 — 교회 선택' : '찬양대원 신청 — 교회 선택';
    return Scaffold(
          ...
          Expanded(child: result.when(
            ...
            error: (e, _) => Center(
              child: Padding(
                ...
                child: Text('검색 실패: $e', style: AppText.body(13, color: AppColors.error)),
              ),
            ),
            data: (churches) => churches.isEmpty
                ? _EmptyState(onCreate: () => Navigator.pushReplacement(
                    context,
    ...
    return Center(
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Icon(Icons.search_off_rounded, size: 56, color: AppColors.muted.withValues(alpha: 0.5)),
          const SizedBox(height: 12),
          Text('찾으시는 교회가 없어요', style: AppText.body(15, weight: FontWeight.w700)),
          const SizedBox(height: 8),
          Text('새 교회로 직접 등록하실 수 있습니다',
              style: AppText.body(13, color: AppColors.muted), textAlign: TextAlign.center),
